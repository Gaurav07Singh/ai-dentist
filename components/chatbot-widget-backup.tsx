...
const handleTimeSelect = (time: string) => {
  ...
  const getBotResponse = async (userInput: string): Promise<string> => {
    ...
    switch (step) {
      ...
      case 'name':
        ...
        setStep('phone')
        return `Nice to meet you, ${userInput}! Could you please provide your phone number so we can contact you?`
      ...
      case 'phone':
        ...
        setStep('doctor')
        return 'Thanks! Which doctor would you like to see? Here are our specialists:\n\n' +
               doctors.map(d => `${d.id}. ${d.name} (${d.specialty})`).join('\n') +
               '\n\nPlease enter the number or name of your preferred doctor.'
      ...
      case 'doctor':
        ...
        if (!selectedDoctor) {
          return 'Sorry, I couldn\'t find that doctor. Please choose from:\n\n' +
                 doctors.map(d => `${d.id}. ${d.name} (${d.specialty})`).join('\n')
        }
        
        setAppointmentData((prev) => ({ ...prev, doctor_id: selectedDoctor.id }))
        setStep('problem')
        return `Great! You've selected ${selectedDoctor.name}. What is the reason for your visit or dental problem?`

      case 'problem':
        setAppointmentData((prev) => ({ ...prev, problem: userInput }))
        setStep('appointmentType')
        return 'I understand. What type of appointment do you need?\n\n1. Consultation\n2. Cleaning\n3. Emergency\n4. Treatment\n5. Follow-up\n\nPlease enter the number or type.'
      ...
      case 'confirmation':
        if (input.includes('yes') || input.includes('confirm')) {
          ...
          try {
            ...
            if (data.success) {
              ...
              return `🎉 Thank you! Your appointment has been successfully booked.\n\n` +
                     ...
                     `Date: ${data.data.date}\n` +
                     `Time: ${data.data.time}\n\n` +
                     `Our clinic will contact you shortly to confirm. We look forward to seeing you at SmileCare Dental Clinic!`
            } else {
              return `Sorry, there was an issue booking your appointment: ${data.error}. Please try again or contact our clinic directly.`
            }
          } catch (error) {
            console.error('Error saving appointment:', error)
            return 'Sorry, there was a technical issue. Please try again or contact our clinic directly.'
          } finally {
            setIsLoading(false)
          }
          ...
          setStep('name')
          setAppointmentData({})
          return 'No problem! Let\'s start over. May I know your name?'
        }
      ...
      default:
        return "I didn't quite understand. Could you please rephrase?"
    }
  }
  ...
  const handleSendMessage = async () => {
    ...
    // Simulate bot typing delay and get response
    try {
